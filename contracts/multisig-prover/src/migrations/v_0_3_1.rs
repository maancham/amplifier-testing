...
pub fn migrate_verifier_sets(
    deps: DepsMut
) -> Result<Response, axelar_wasm_std::ContractError> {
    let current_worker_set = CURRENT_WORKER_SET.load(deps.storage)?;
...
#[cfg(test)]
mod test {
use crate::{migrations::v_0_3_1::NEXT_WORKER_SET, state::{CURRENT_VERIFIER_SET, NEXT_VERIFIER_SET}, test::test_data::new_worker_set};
    ...
    fn should_be_able_to_migrate_worker_set_to_verifier_set() {
        ...
        CURRENT_WORKER_SET.save(&mut deps.storage, &worker_set).unwrap();
        ...
        assert!(NEXT_VERIFIER_SET.may_load(&mut deps.storage).unwrap().is_none());

        assert!(CURRENT_WORKER_SET.may_load(&mut deps.storage).unwrap().is_none());
        assert!(NEXT_WORKER_SET.may_load(&mut deps.storage).unwrap().is_none());
    }
    ...
    fn should_be_able_to_migrate_worker_set_to_verifier_set_mid_rotation() {
        ...
        next_worker_set.threshold = worker_set.threshold.checked_add(Uint256::one()).unwrap();
        NEXT_WORKER_SET.save(&mut deps.storage, &next_worker_set).unwrap();
        let res = migrate_verifier_sets(deps.as_mut());
        assert!(res.is_ok());
        let verifier_set = CURRENT_VERIFIER_SET.load(&mut deps.storage).unwrap();
        assert_eq!(verifier_set, worker_set);
